        ...
        public void _codes_04hSuccess()
        {
            ...
            try
            {
                foreach (var eventAsByte in eventAsBytes)
                {
                    ...
                    {
                        if (eventPutDTOs[i].EventNameId == 9)
                        {
                            var @event = eventPutDTOs[i];
                            res = CommonFunctions.FinishClientSucceed(ref @event);
                        }
                        else
                        {
                            throw new Exception("EventPostProcessing _codes_04hSuccess function failed: eventPutDTOs[i].EventNameId eventCode not matched: " + eventPutDTOs[i].EventHex);
                        }
                    }
                    else
                    {
                        throw new Exception("EventPostProcessing _codes_04hSuccess function failed: eventPutDTO eventCode is null: " + eventPutDTOs[i].EventHex);
                    }
                    ++i;
                }
            }
            ...
            {
                throw new Exception("EventPostProcessing _codes_04hSuccess function failed: eventPutDTO eventCode is null: " + ex.Message);
            }

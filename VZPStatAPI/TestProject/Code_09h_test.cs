...
namespace TestProject
{
    public class Code_09h_test
    {
        ...
        [Test]
        public void _codes_09h_Success()
        {
            // Arrange
            ...
            try
            {
                foreach (var eventAsByte in eventAsBytes)
                {
                    ...
                    {
                        if (eventPutDTOs[i].EventNameId == 17)
                        {
                            var @event = eventPutDTOs[i];
                            res = CommonFunctions.ChangePrinterStatusSucceed(ref @event);
                        }
                        else
                        {
                            throw new Exception("EventPostProcessing _codes_09h_Success function failed: eventPutDTOs[i].EventNameId eventCode not matched: " + eventPutDTOs[i].EventHex);
                        }
                    }
                    else
                    {
                        throw new Exception("EventPostProcessing _codes_09h_Success function failed: eventPutDTO eventCode is null: " + eventPutDTOs[i].EventHex);
                    }
                    ++i;
                }
            }
            ...
            {
                throw new Exception("EventPostProcessing _codes_09h_Success function failed: eventPutDTO eventCode is null: " + ex.Message);
            }
